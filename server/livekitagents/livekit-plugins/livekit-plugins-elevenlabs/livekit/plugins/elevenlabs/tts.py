...
def _to_timed_words(
    ...
    """
    Convert ElevenLabs character-level alignment into character-level TimedStrings.

    Returns:
        (emitted_chars, remaining_text)
    """
    if not text:
        return [], ""

    # Make sure arrays are consistent with the text buffer length
    n = min(len(text), len(start_times_ms), len(durations_ms))
    if n == 0:
        return [], text

    # Build N+1 timestamps: one for each char boundary
    timestamps = start_times_ms[:n] + [start_times_ms[n - 1] + durations_ms[n - 1]]

    # If we're not flushing, hold back the last character so that
    # it can be extended by the next chunk (same idea as the original
    # "leave last word as remaining" logic).
    last_idx = n if flush else max(n - 1, 0)

    timed_chars: list[TimedString] = []
    for i in range(last_idx):
        start_t = timestamps[i] / 1000.0
        end_t = timestamps[i + 1] / 1000.0
        # one TimedString per character
        timed_chars.append(
            TimedString(
                text=text[i : i + 1],
                start_time=start_t,
    ...
    return timed_chars, remaining_text
