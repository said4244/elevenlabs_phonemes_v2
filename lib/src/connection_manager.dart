...
class ConnectionManager {
  ...
  Future<Map<String, String>> _getConnectionInfo() async {
    ...
    try {
      ...
      VoiceAssistantLogger.debug(
          'Token server response headers: ${response.headers}');
      ...
      if (response.statusCode == 200) {
        ...
        if (token.isEmpty) {
          throw Exception(
              'Token server returned empty token. Response: ${response.body}');
        }
      ...
      VoiceAssistantLogger.error(
          'Failed to get connection token', e, stackTrace);
      if (e.toString().contains('XMLHttpRequest error')) {
        ...
        VoiceAssistantLogger.error(
            '3. You\'re using --web-browser-flag "--disable-web-security" for local dev');
      }
      rethrow;
    }
  }
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      VoiceAssistantLogger.info(
          'Local participant SID: ${_room!.localParticipant?.sid}');
      ...
      VoiceAssistantLogger.error(
          'Connection failed with error: ${e.runtimeType}', e, stackTrace);
        ...
        try {
          ...
          try {
            ...
            if (dataString.isNotEmpty) {
              VoiceAssistantLogger.debug(
                  'Plain text transcription: $dataString');
              _notifyEvent('transcription', {
                'text': dataString,
                'timestamp': DateTime.now().millisecondsSinceEpoch,
              });
          ...
          VoiceAssistantLogger.debug(
              'Received non-text data: ${event.data.length} bytes');
        }
      })
