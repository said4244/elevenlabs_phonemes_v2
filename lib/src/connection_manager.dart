      ...
      ..on<DataReceivedEvent>((event) {
        try {
          ...
          try {
            ...
            if (data is Map && data.containsKey('type') && data['type'] == 'transcription') {
              final text = data['text'] as String?;
              if (text != null) {
                final startTime = data['start_time'] as num?;
                final endTime = data['end_time'] as num?;
                VoiceAssistantLogger.debug('Transcription char: "$text" [${startTime}s -> ${endTime}s]');
            ...
            if (dataString.isNotEmpty) {
              VoiceAssistantLogger.debug('Plain text transcription: $dataString');
              _notifyEvent('transcription', {
                'text': dataString,
                'timestamp': DateTime.now().millisecondsSinceEpoch,
              });
          ...
          VoiceAssistantLogger.debug('Received non-text data: ${event.data.length} bytes');
        }
      })
